...
"""Point-in-time safe evidence and market-data helpers for backtesting.
...
class TemporalEvidence:
    """Read canonical evidence rows with point-in-time filtering."""
    ...
    def search_for_context(
        ...
        *,
        ...
        parts = [f"=== Temporal evidence as_of={normalize_ymd(as_of_date)} theme={self.theme_key} ==="]
...
def build_period_snapshot(
    *,
    ...
    evidence = TemporalEvidence(data_dir=data_dir, theme_key=theme_key)
    rows = evidence.filter_period_records(
        from_date=from_date,
        to_date=to_date,
        source_types=source_types,
    )
    ...
    if build_vector:
        ...
        vector_stats = SourceevidenceBuilder().upsert_by_source(
            records=rows,
