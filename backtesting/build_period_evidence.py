...
"""Build a cached period evidence snapshot for faster backtest loops."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"[PERIOD evidence] output: {result['output_dir']}")
    print(f"[PERIOD evidence] combined: {result['combined_count']}")
    for source, count in sorted(result["source_counts"].items()):
        print(f"  - {source}: {count}")
    if result["vector_stats"]:
        print("[PERIOD evidence] vector stores:")
