...
# - Consumes data from data/canonical_index/<theme>/ (synced by EvidenceIndexBuilder).
# - Falls back to src/retrieval for pipeline-level search.

"""Canonical evidence retriever — unified text retrieval for agent runtime."""
...
from src.evidence.source_weighting import (
...
class EvidenceRetriever:
    ...
    Usage:
        retriever = EvidenceRetriever(data_dir="./data")
    ...
    def search(
        ...
        Args:
            ...
            stock_code: If set, use metadata-first retrieval for this stock only
        ...
        if stock_code:
            stock_results = self._search_stock_documents(
                themes=themes,
                stock_code=stock_code,
                top_k=top_k,
                source_filter=effective_source_filter,
            )
            return apply_source_weighting(stock_results)[:top_k]
    ...
    def search_for_context(
        ...
        parts = ["=== 검색된 문서 (Canonical evidence) ==="]
    ...
    def _search_stock_documents(
        ...
        results: List[Dict] = []
        for theme in themes:
            for row in self._load_corpus(theme):
                meta = row.get("metadata", {}) or {}
                if str(meta.get("stock_code", "")).strip() != normalized_stock_code:
                    continue
                ...
                results.append(
                    {
                        ...
                        "score": 1.0,
                        "theme_key": theme,
                        "retrieval_method": "stock_metadata",
                    }
                )

        results.sort(
            key=lambda row: (
                get_source_weight(str(row.get("source_type", ""))),
                str((row.get("metadata") or {}).get("published_at", "")),
            ),
            reverse=True,
        )
        return results[:top_k]
