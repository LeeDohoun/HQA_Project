...
class DartDisclosureCollector(BaseCollector):
    ...
    def collect(
        ...
        for item in payload.get("list", []):
            ...
            print(
                f"[DART:DEBUG] rcept_no={rcept_no} final_path={body_source or 'title_fallback'} "
                f"has_body={has_body} error_type={body_error_type}"
            )
    ...
    def _fetch_structured_body(
        ...
        print(f"[DART:DEBUG] rcept_no={rcept_no} structured_candidates={endpoints}")
        if not endpoints:
        ...
        for endpoint in endpoints:
            ...
            if payload is None:
                try:
                    print(f"[DART:DEBUG] structured_api_hit endpoint={endpoint} corp_code={corp_code}")
                    response = self.get_with_retry(
            ...
            target, matched_rcept = self._find_structured_row(rows=rows, rcept_no=rcept_no)
            print(
                f"[DART:DEBUG] structured_rcept_match endpoint={endpoint} "
                f"rcept_no={rcept_no} matched={matched_rcept}"
            )
            if target is None:
            ...
            if len(cleaned) < 120:
                ...
                continue
            quality_meta["body_error_type"] = "success"
    ...
    def _find_structured_row(self, rows: List[Dict], rcept_no: str) -> Tuple[Dict | None, bool]:
        if not rows:
            return None, False
        if rcept_no:
            for row in rows:
