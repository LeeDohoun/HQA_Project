...
HQA는 한국 주식 분석을 위한 AI + 데이터 통합 실행 프로젝트입니다. 이 브랜치는 데이터 수집/검색 자산과 멀티 에이전트 분석 로직을 한 실행 단위로 묶고, RAG 기반 질의응답, 종목 분석, 테마 주도주 선별, 백테스팅, dry-run 거래 판단, 한국투자증권 모의투자 주문 전송을 제공합니다.
...
- 거래 API는 dry-run 시뮬레이션과 한국투자증권 KIS 모의투자 주문 전송을 지원합니다.
- 실전 계좌 주문은 기본 차단되어 있으며, `allow_real_trading=true`를 명시해야만 전송됩니다.
...
- 프롬프트는 뽀삐뽀삐보 투자교본의 시장/산업/대장주/차트/비중/매도 원칙을 에이전트별 역할에 맞게 반영합니다.

### 투자교본 기반 프롬프트

`prompts/` 아래의 에이전트 프롬프트는 일반적인 재무/차트 분석을 넘어 주도주 중심의 판단 기준을 사용합니다.

| 에이전트 | 프롬프트 | 반영된 투자교본 원칙 |
|---|---|---|
| Supervisor | `prompts/supervisor/routing.md` | 헤게모니, 주도주, 대장주, 쉬운 구간, 비중/손절/매도 질문 라우팅 |
| Analyst | `prompts/analyst/analysis.md` | 산업 헤게모니 4국면, 대장주 여부, 약한 후발주 배제, 주도성 약화 신호 |
| Quant | `prompts/quant/web_fallback.md` | 저평가 함정 경계, 대장주 프리미엄, 실적/수주/마진 개선 확인 |
| Chartist | `prompts/chartist/analysis.md` | 추세/채널/지지저항, 신고가, 조정률, 거래대금, 쉬운 구간 |
| RiskManager | `prompts/risk_manager/decision.md` | 매수 전 질문, 비중 단계, 시나리오 실패 매도, 리스크 관리 매도, 헤게모니 약화 매도 |

교본 원칙은 판단을 강제하는 규칙이 아니라 데이터 해석의 기준입니다. 리서치 품질이 낮거나 에이전트 신호가 충돌하면 `RiskManager`는 더 보수적인 결론을 선택하도록 되어 있습니다.
...
- `dry_run=false`, `account_type=paper`이면 한국투자증권 KIS 모의투자 API로 국내주식 현금 주문을 전송합니다.
- `dry_run=false`, `account_type=real`, `allow_real_trading=true`이면 KIS 실전 계좌로 국내주식 현금 주문을 전송합니다.
- `order_type=limit`은 현재가 기반 지정가 주문, `order_type=market`은 시장가 주문으로 전송합니다.
- 자동 매도는 KIS 잔고조회로 주문 가능 수량을 확인한 뒤 보유 수량이 있을 때만 전송합니다.
...
`KIS_PAPER_*` 값이 있으면 모의투자 주문에서 우선 사용합니다. 값이 없을 때는 기존 `KIS_APP_KEY`, `KIS_APP_SECRET`, `KIS_ACCOUNT_NO`로 fallback하지만, 실전 계좌 주문은 `config/watchlist.yaml`의 `allow_real_trading: true`가 없으면 차단됩니다.
...
실전 주문은 `.env`만으로는 열리지 않습니다. `config/watchlist.yaml`에서 `dry_run: false`, `account_type: "real"`, `allow_real_trading: true`를 함께 명시해야 전송됩니다.
...
`dry_run_override=true`이면 실제 주문을 보내지 않고 simulated 로그만 남깁니다. KIS 모의투자 주문 전송까지 확인하려면 `.env`에 `KIS_PAPER_APP_KEY`, `KIS_PAPER_APP_SECRET`, `KIS_PAPER_ACCOUNT_NO`를 넣고 `dry_run_override=false`, `trading_enabled_override=true`로 호출합니다.
...
```json
{
  ...
  "final_decision": {
    ...
    "risk_level": "낮음",
    "risk_level_code": "LOW",
    "summary": "KIS 모의투자 주문 테스트"
  }
}
```

기본 설정은 `config/watchlist.yaml`의 `trading.enabled=false`, `dry_run=true`, `account_type=paper`, `order_type=limit`, `allow_real_trading=false`입니다. KIS 주문은 한국투자증권 국내주식 현금 주문 API와 `hashkey`를 사용하며, 성공 응답의 주문번호와 주문시각을 주문 로그에 함께 저장합니다. 매도 판단은 KIS 잔고조회 API로 주문 가능 수량을 확인한 뒤 실행합니다.
...
- `config/watchlist.yaml`의 `trading.enabled=false`, `dry_run=true`, `account_type=paper`, `allow_real_trading=false`가 안전 기본값입니다.
- 한국투자증권 모의투자 주문은 국내주식 현금 주문 기준입니다. 계좌 상품 코드, 주문 가능 시간, 모의투자 권한, 종목별 주문 제한은 KIS 계정 상태에 따라 달라질 수 있습니다.
- 실전 계좌 자동매매는 `allow_real_trading=true`로 열 수 있지만, 운영 전에는 소액 모의투자와 주문 로그를 먼저 검증하세요.
