...
def token():
    if os.getenv("RUN_KIS_LIVE_TESTS") != "1":
        pytest.skip("set RUN_KIS_LIVE_TESTS=1 to run live KIS paper trading checks")
    if not KEY or not SECRET or not CANO:
        pytest.skip("KIS paper credentials are not configured")
    issued = issue_token()
    if not issued:
        pytest.skip("KIS paper token could not be issued")
    return issued
