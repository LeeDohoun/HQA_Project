...
def test_temporal_evidence_filters_future_documents(tmp_path):
    ...
    evidence = TemporalEvidence(data_dir=str(tmp_path), theme_key="ai")
    rows = evidence.filter_records("20250201", source_types=["news"])
    assert len(rows) == 1
    assert rows[0]["metadata"]["title"] == "old"


def test_temporal_evidence_applies_forum_lookback(tmp_path):
    from backtesting import TemporalEvidence
    ...
    evidence = TemporalEvidence(data_dir=str(tmp_path), theme_key="ai")
    rows = evidence.filter_records("20250401", source_types=["forum"], lookback_days={"forum": 30})
    assert len(rows) == 1
...
def test_build_period_snapshot_writes_filtered_sources(tmp_path):
    from backtesting.temporal_evidence import build_period_snapshot
...
def test_clean_period_evidence_keeps_dart_chunks_and_reduces_noisy_duplicates():
    from backtesting.clean_period_evidence import clean_rows
