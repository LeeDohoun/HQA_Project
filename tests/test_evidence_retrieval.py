"""
Canonical evidence architecture tests — behavioral verification.
...
class TestSourceMetadata:
    ...
    def test_rag_corpus_builder_canonical_fields(self):
        ...
        builder = EvidenceCorpusBuilder(chunk_size=100, chunk_overlap=20)
        docs = [
            DocumentRecord(
                source_type="dart",
                title="TEST 공시",
...
class TestSourceWeighting:
    ...
    def test_source_weights_hierarchy(self):
        from src.evidence.source_weighting import get_source_weight
    ...
    def test_freshness_multiplier_recent_news(self):
        ...
        from src.evidence.source_weighting import compute_freshness_multiplier
    ...
    def test_apply_source_weighting_sorts_correctly(self):
        from src.evidence.source_weighting import apply_source_weighting
...
class TestAnalystSourceFilter:
    def test_analyst_has_source_aware_tools(self):
        ...
        agent = AnalystAgent()
        assert hasattr(agent, "evidence_tool_evidence")
        assert hasattr(agent, "evidence_tool_news")
        assert not hasattr(agent, "evidence_tool_reports")
    ...
    def test_evidence_tool_filters_sources(self):
        ...
        agent = AnalystAgent()
        tool = agent.evidence_tool_evidence
    ...
    def test_full_analysis_returns_analyst_score_directly(self):
        ...
        agent._search_evidence = lambda stock_name, stock_code="": ("DART 투자 근거", ["dart"])
        agent._search_news = lambda stock_name, stock_code="": "뉴스 심리"
    ...
    def test_intent_source_map_coverage(self):
        from src.evidence.source_weighting import INTENT_SOURCE_MAP
...
class TestChartPathNonRegression:
    ...
    def test_market_sources_excluded_from_document(self):
        from src.evidence.source_registry import is_document_source, is_market_source
    ...
    def test_chartist_does_not_use_canonical_rag(self):
        ...
        assert "EvidenceRetriever" not in source
...
class TestMetadataPromotion:
    ...
    def test_theme_key_preserved(self):
        ...
        builder = EvidenceCorpusBuilder(chunk_size=500, chunk_overlap=50)
        docs = [
            DocumentRecord(
                source_type="news",
                title="Test",
...
class TestCanonicalFallback:
    """EvidenceRetriever must gracefully handle missing data."""
    ...
    def test_search_with_stock_code_filter_uses_metadata_first(self):
        """Stock filter must return only records for the requested stock_code."""
        from src.evidence.retriever import EvidenceRetriever
        from src.retrieval.vector_store import SimpleVectorStore
        from src.retrieval.bm25_index import BM25IndexManager

        with tempfile.TemporaryDirectory() as tmpdir:
            index_dir = Path(tmpdir) / "canonical_index" / "semiconductor"
            index_dir.mkdir(parents=True)

            records = [
                {
                    "text": "삼성전자는 HBM 공급 확대와 메모리 업황 회복 기대가 있다.",
                    "metadata": {
                        "source_type": "news",
                        "stock_code": "005930",
                        "stock_name": "삼성전자",
                ...
                {
                    "text": "SK하이닉스도 HBM 시장 점유율 확대가 기대된다.",
                    "metadata": {
                        "source_type": "news",
                        "stock_code": "000660",
                        "stock_name": "SK하이닉스",
            ...
            store = SimpleVectorStore()
            store.add_texts([r["text"] for r in records], [r["metadata"] for r in records])
            store.save(str(index_dir / "combined_vector_store.json"))

            bm25 = BM25IndexManager(
                persist_path=str(index_dir / "bm25_index.json"),
                auto_save=False,
            )
            bm25.add_texts([r["text"] for r in records], [r["metadata"] for r in records])
            bm25.save_index()
            ...
            context = retriever.search_for_context(
                "HBM 전망",
                source_types=["news"],
                stock_code="005930",
                top_k=10,
            ...
            assert results
            assert {row["metadata"]["stock_code"] for row in results} == {"005930"}
            assert "삼성전자 HBM 전망" in context
            assert "SK하이닉스 HBM 전망" not in context
...
class TestRetrieverTypeContract:
    """get_retriever() must always return EvidenceRetriever."""

    def test_get_retriever_returns_canonical(self):
        from src.tools.evidence_tool import get_retriever
        from src.evidence.retriever import EvidenceRetriever
    ...
    def test_get_canonical_retriever_singleton(self):
        from src.tools.evidence_tool import get_canonical_retriever
