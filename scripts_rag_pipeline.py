...
def main() -> None:
    ...
    document_sources = ["news", "dart", "forum"]
    market_sources = ["chart", "quote"]

    raw_document_count = sum(
        sum((r.get("document_counts") or {}).get(source, 0) for source in document_sources)
        for r in run_reports
    )
    raw_market_record_count = sum(
        sum((r.get("market_data_counts") or {}).get(source, 0) for source in market_sources)
        for r in run_reports
    )
    raw_saved_count_total = sum(
    ...
    source_status = {
        source: {
            "collector_ok_count": sum(
                1 for r in run_reports
                if (r.get("collector_ok") or r.get("collector_status") or {}).get(source)
            ),
            "data_present_count": sum(
                1 for r in run_reports
                if (r.get("data_present") or r.get("source_data_present") or {}).get(source)
            ),
            "warning_samples": [
                r.get("warnings", {}).get(source)
                for r in run_reports
                if r.get("warnings", {}).get(source)
            ][:5],
            "failure_samples": [
                r.get("failures", {}).get(source)
                for r in run_reports
                if r.get("failures", {}).get(source)
            ][:5],
        }
    ...
    missing_stocks = [
        r["stock_name"]
        for r in run_reports
        if (sum((r.get("document_counts") or {}).values()) + sum((r.get("market_data_counts") or {}).values())) == 0
    ]
    summary_report = {
        "theme_key": theme_key,
        ...
        "source_collector_ok": {k: v["collector_ok_count"] for k, v in source_status.items()},  # deprecated
        "source_data_present": {k: v["data_present_count"] for k, v in source_status.items()},  # deprecated
        "source_success": {  # deprecated: collector_ok AND data_present count
            source: sum(
                1 for r in run_reports
                if ((r.get("collector_ok") or r.get("collector_status") or {}).get(source))
                and ((r.get("data_present") or r.get("source_data_present") or {}).get(source))
            )
            for source in enabled_sources
        },
        "source_fail": {
        ...
        "raw_market_record_count": raw_market_record_count,
        "raw_docs_count": raw_docs_count,  # deprecated
        "raw_saved_count_total": raw_saved_count_total,
        "raw_saved_count": raw_saved_count_total,  # deprecated
        "skipped_invalid_count_by_source": skipped_invalid_count_by_source,
        "built_document_records_count": built_records_count,
        "final_document_records_count": final_records_count,
        "built_records_count": built_records_count,  # deprecated
        "final_records_count": final_records_count,  # deprecated
    ...
    print(f"  - raw_document_count: {summary_report['raw_document_count']}")
    print(f"  - raw_market_record_count: {summary_report['raw_market_record_count']}")
    print(f"  - raw_saved_count_total: {summary_report['raw_saved_count_total']}")
    print(f"  - skipped_invalid_count_by_source: {summary_report['skipped_invalid_count_by_source']}")
    print(f"  - source_status: {summary_report['source_status']}")
    print(f"  - built_document_records_count: {summary_report['built_document_records_count']}")
    print(f"  - final_document_records_count: {summary_report['final_document_records_count']}")
    if missing_stocks:
